...
public class FlowDrawAction extends AbstractGameAction {
    ...
    private void endAction() {
        ...
        for (AbstractRelic relic : AbstractDungeon.player.relics) {
            if (relic instanceof FlowAffectingRelic) {
                leftovers += ((FlowAffectingRelic)relic).leftoversToKeep(power.amount);
            }
        }
        if (leftovers == 0) {
            addToTop(new RemoveSpecificPowerAction(AbstractDungeon.player, AbstractDungeon.player, power));
        } else if (leftovers < power.amount) {
            addToTop(new ReducePowerAction(AbstractDungeon.player, AbstractDungeon.player, power, power.amount - leftovers));
        } else {
